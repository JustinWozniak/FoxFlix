...
const highestRatedR = "https://api.themoviedb.org/3/discover/movie?api_key=" + process.env.REACT_APP_API_KEY + "&certification_country=US&certification=R&sort_by=vote_average.desc&include_adult=true"

// Then we'll fetch user data from this API
const ratedRTask = async () =>
  await fetch(highestRatedR)
    .then(res => (res.ok ? res : Promise.reject(res)))
    .then(res => res.json())



function RatedR(props) {
 
  const [componentType, setLink] = useState("movie/")
  const posterPath = "https://image.tmdb.org/t/p/w500/"
  const { data, error, isLoading } = useAsync({ promiseFn: ratedRTask })
  ...
  if (data) {
    ...
    return (<landingContext.Consumer>
      {passedLink => (
        <div>
          <h2 className="headerOne">Best Rated R:</h2>
          {
