...
const MostPopularTvLink = "https://api.themoviedb.org/3/tv/popular?api_key=" + process.env.REACT_APP_API_KEY + "&language=en-US&page=1"

// Then we'll fetch user data from this API
const mostPopularTask = async () =>
  await fetch(MostPopularTvLink)
    .then(res => (res.ok ? res : Promise.reject(res)))
    .then(res => res.json())



function MostPopularTv(props) {
 
  const [componentType, setLink] = useState("tv/")
  const posterPath = "https://image.tmdb.org/t/p/w500/"
  const { data, error, isLoading } = useAsync({ promiseFn: mostPopularTask })
  if (isLoading) return "Loading..."
  if (error) return `Something went wrong: ${error.message} Most Popular`
  if (data) {
    ...
    return (<landingContext.Consumer>
      {passedLink => (
        <div>
          <h2 className="headerOne">Popular Shows:</h2>
          {
