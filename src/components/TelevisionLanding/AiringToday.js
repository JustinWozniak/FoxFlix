...
const airingTodayShows = "https://api.themoviedb.org/3/tv/airing_today?api_key=" + process.env.REACT_APP_API_KEY + "&language=en-US&page=1"

// Then we'll fetch user data from this API
const airingTodayTask = async () =>
  await fetch(airingTodayShows)
...
function AiringToday(props) {
  ...
  const { data, error, isLoading } = useAsync({ promiseFn: airingTodayTask })
  ...
  if (data) {
    ...
    return (<landingContext.Consumer>
      {passedLink => (
        <div>
          <h2 className="headerOne">Airing Tonight:</h2>
          {
