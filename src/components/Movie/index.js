...
const popularActorsTask = async () =>
    ...
    await fetch("https://api.themoviedb.org/3/movie/1878?api_key=" + process.env.REACT_APP_API_KEY + "&language=en-US&append_to_response=credits")
            ...
            <div style={{ backgroundImage: data.backdrop_path }}>
                ...
                className="movies"
                    ...
                    {actors.map((images, index) => {
                        return <div><img className="actorMediumImages" key={index} src={posterPath + actors[index].profile_path} alt={index} />
                            <hr />
